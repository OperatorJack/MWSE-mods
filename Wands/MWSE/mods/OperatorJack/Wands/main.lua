...
local function onAttack(e)
    ...
    if (config.wands[weapon.object.mesh:lower()]) then
        debug("Using Wand.")
        continue = true
    elseif (config.applyToStaves == true and weapon.object.type ==
        tes3.weaponType.bluntTwoWide) then
        debug("Using Staff.")
        continue = true
    end
    ...
    for i = 1, #weapon.object.enchantment.effects do
        ...
        local magicEffect = tes3.getMagicEffect(enchantmentEffect.id)
        local name = ""
        if (magicEffect) then
        ...
        effect.name = name
        effect.range = enchantmentEffect.rangeType
    ...
    if (#effects == 0) then
        debug("No suitable effects found. Returning...")
        return
    end
