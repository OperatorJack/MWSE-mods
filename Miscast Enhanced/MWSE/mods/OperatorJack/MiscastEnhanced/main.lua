...
if (mwse.buildDate == nil) or (mwse.buildDate < 20190821) then
    event.register("initialized", function()
        tes3.messageBox("[Miscast Enhanced] Your MWSE is out of date!" ..
...
if (framework == nil) then
    local function warning()
        tes3.messageBox("[Miscast Enhanced: ERROR] Magicka Expanded framework is not installed!" ..
...
event.register("modConfigReady", function() require("OperatorJack.MiscastEnhanced.mcm") end)
...
local function onSpellCastedFailure(e)
    ...
    if (chance > 15 and functions.isDebug() == false) then return end
