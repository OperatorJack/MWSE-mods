...
event.register("activate", function(e)
    if (e.activator == tes3.player) then if (isActivationDisabled == true) then return false end end
end, {priority = 1})
...
event.register("simulate", function()
    ...
    if (isWatersinkEnabled == true and tes3.mobilePlayer.underwater == true) then
...
local effects = {
    ...
    [tes3.effect.almsiviIntervention] = function(params)
        functions.handlers.genericTeleportEffectHandler({reference = params.caster})
    end,
    ...
    [tes3.effect.charm] = function(params)
        ...
        local actors = framework.functions.getActorsNearTargetPosition(params.caster.cell,
                                                                       params.caster.position,
                                                                       distance)
    ...
    [tes3.effect.detectAnimal] = function(params)
        ...
        for _, actor in pairs(actors) do
            if (actor.object.objectType == tes3.creatureType.normal) then
                mwscript.startCombat({reference = actor, target = params.caster})
            end
        end
    ...
    [tes3.effect.slowFall] = function(params)
        if (params.caster == tes3.player) then
            local duration = functions.getModifiedDurationFromEffect(params.effect)
            functions.gatedMessageBox(
                "Your body feels more dense, as through it moves through the air more easily.")
    ...
    [tes3.effect.waterBreathing] = function(params)
        if (params.caster == tes3.player and isBreathingDisabled == false) then
            ...
            timer.start({
                ...
                callback = function()
                    if (isBreathingDisabled == true and tes3.mobilePlayer.underwater == false) then
...
local function onRegister()
    for effect, handler in pairs(effects) do functions.setEffectHandler(effect, handler) end
end
