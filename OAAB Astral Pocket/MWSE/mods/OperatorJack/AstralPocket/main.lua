...
if (mwse.buildDate == nil) or (mwse.buildDate < 20190821) then
    local function warning()
        tes3.messageBox("[Astral Pocket ERROR] Your MWSE is out of date!" ..
...
if (framework == nil) then
    local function warning()
        tes3.messageBox("[Astral Pocket ERROR] Magicka Expanded framework is not installed!" ..
...
local function onTeleportToAstralPocketTick(e)
    ...
    if canTeleport then
        ...
        orientationRad[1] = math.rad(common.cell.orientation.x)
        orientationRad[2] = math.rad(common.cell.orientation.y)
        orientationRad[3] = math.rad(common.cell.orientation.z)
...
local function readAstrologicalElements(e)
    ...
    if tes3.mobilePlayer.mysticism.current > 80 then
        local teleportToAstralPocketSpell = tes3.getObject(common.spellIds.TeleportToAstralPocket)
        ---@cast teleportToAstralPocketSpell tes3spell

        if (teleportToAstralPocketSpell ~= nil and
            not tes3.player.object.spells:contains(teleportToAstralPocketSpell)) then
            tes3.updateJournal({id = common.journalIds.QuestOne, index = 20})
            tes3.addSpell({
                reference = player,
                spell = teleportToAstralPocketSpell,
                updateGUI = true
            })
        end
    else
        tes3.messageBox(
            "You try to learn the spell to teleport to the Astral Pocket, but you find you are not skilled enough in the art of Mysticism.")
    end
end

local function readNelasNote(e)
    local questOneIndex = tes3.getJournalIndex({id = common.journalIds.QuestOne}) or -1
    if questOneIndex < 30 then tes3.updateJournal({id = common.journalIds.QuestOne, index = 30}) end
end
---------------------------

-- Initilization Section --
local function initialized(e)
    if not tes3.isModActive("OAAB - Astral Pocket.ESP") then
