...
if (mwse.buildDate == nil) or (mwse.buildDate < 20190821) then
    local function warning()
        tes3.messageBox("[Chrysopoeia ERROR] Your MWSE is out of date!" ..
...
if (framework == nil) then
    local function warning()
        tes3.messageBox("[Chrysopoeia ERROR] Magicka Expanded framework is not installed!" ..
...
-- Function Section --
local function calculateItemValue(item) return item.value end

local function filterItems(e) return calculateItemValue(e.item) > 0 end
...
local function getMultiplier()
    ...
    if (multiplier > .85) then multiplier = .85 end
...
local function onInventoryItemSelected(e)
    ...
    tes3.mobilePlayer:applyDamage({damage = damageAmount})
...
local function readChrysopoeia(e)
    ...
    ---@cast chrysopoeiaSpell tes3spell
    ...
    if (not tes3.player.object.spells:contains(chrysopoeiaSpell)) then
        ...
        tes3.addSpell({reference = player, spell = chrysopoeiaSpell})
    end
end
