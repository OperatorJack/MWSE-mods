...
local spellIds = {flotation = "OJ_ME_FlotationSpell"}
...
event.register("initialized", function()
    ...
    local function levitate()
        if (tes3.menuMode() == true) then return end
    ...
    local function stage20Event()
        ...
        local tarhiel = tes3.getReference(tarhielId)
        mwscript.equip({reference = tarhiel, item = levitationPotionId})
        tes3.playAnimation({
            reference = tarhiel,
            ...
            callback = function()
                ...
                timer.start({
                    ...
                    callback = function()
                        ...
                        tes3.removeEffects({effect = tes3.effect.levitate, reference = tarhiel})
    ...
    mwse.overrideScript("OJGA_ASpaceOdyssey20Script", function(e)
        local globalId = "OJGA_ASpaceOdyssey20Global"
        local global = tes3.findGlobal(globalId)
        if (global.value == 0) then event.register("simulate", stage20Event) end
    ...
    local function finalEvent(e)
        ...
        local tarhiel = tes3.getReference(tarhielId)
        tes3.cast({reference = tarhiel, target = tarhiel, spell = spellIds.flotation})
        local state = tes3.mobilePlayer.controlsDisabled
        tes3.mobilePlayer.controlsDisabled = true
        ...
        timer.start({
            ...
            callback = function()
                ...
                timer.start({
                    duration = 1,
                    callback = function()
                        tes3.removeEffects({effect = tes3.effect.float, reference = tarhiel})
                        tarhiel:disable()
                        tes3.fadeIn()
                        tes3.mobilePlayer.controlsDisabled = state
                    end
    ...
    event.register("journal", function(e)
        if (e.topic.id == "OJGA_ASpaceOdyssey") then
            if (e.index == 50) then event.register("simulate", finalEvent) end
        end
    end)
end)
